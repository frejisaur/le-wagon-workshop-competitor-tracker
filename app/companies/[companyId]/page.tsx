...
export default async function CompanyPage({params, searchParams}: {params: Promise<{companyId: string}>; searchParams: Promise<Record<string, string | string[] | undefined>>}) {
  ...
  return <AppShell status={company.status} freshness={company.freshness} breadcrumb={company.identity.displayName ?? company.identity.domain}><CompanyWorkspace company={company} initialTab={tab} initialSearch={search ? `?${search}` : ''} /></AppShell>;
}
