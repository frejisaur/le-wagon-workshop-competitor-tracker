...
export function CompanyLeaderboard({companies, selectedCompany, sort, onSort, onSelect}: {companies: readonly CompanySummary[]; selectedCompany?: string; sort: LandscapeSort; onSort: (sort: LandscapeSort) => void; onSelect: (companyId: string) => void}) {
  useEffect(() => { if (selectedCompany) document.getElementById(`leaderboard-row-${selectedCompany}`)?.focus(); }, [selectedCompany]);
