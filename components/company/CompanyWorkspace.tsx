...
import {workflowStatusLabel} from '@/components/shared/WorkflowStatus';
import {CompanyResponseSchema, type CompanyResponse} from '@/lib/domain/dashboard';
import {AiPresence, AiSummary} from './AiPresence';
...
import {companyTabs, parseCompanyTab, serializeCompanyTab, type CompanyTab} from './company-tab';
import {CompanySwitcher} from './CompanySwitcher';
import {exactUtc} from './company-utils';
import styles from './company.module.scss';
import enhancementStyles from './company-enhancements.module.scss';

export {companyTabs, parseCompanyTab, serializeCompanyTab};
...
export function CompanyWorkspace({company, initialTab = 'overview', initialSearch, comparison}: {company: CompanyResponse; initialTab?: CompanyTab; initialSearch?: string; comparison?: ComparisonDataset}) {
  ...
  const completedOutcome = company.status === 'partial' || company.status === 'failed' || company.status === 'stale';
  return <ScreenState status={company.status} hasRetainedData={retained} recoveryMessage={company.recoveryMessage} announce={completedOutcome}><article className={`${styles.companyWorkspace} company-workspace`}><header className={`${enhancementStyles.heading} company-workspace__heading`}><div className={`${enhancementStyles.identity} company-workspace__identity`}><h1 className="page-title">{company.identity.displayName ?? company.identity.domain}</h1><p>{company.identity.domain}{company.identity.segment ? ` · ${company.identity.segment}` : ''}{company.identity.country ? ` · ${company.identity.country}` : ''}</p><div className={`${enhancementStyles.metadata} company-workspace__metadata`}><span>Enriched {exactUtc(company.enrichedAt)}</span><span data-status={company.status}>Status: {workflowStatusLabel(company.status)}</span></div></div>{comparisonOptions.length ? <CompanySwitcher options={comparisonOptions} tab={tab} /> : null}</header><div className={`${enhancementStyles.tabs} company-workspace__tabs`}><Tabs selectedIndex={selectedIndex} onChange={({selectedIndex: next}) => select(tabs[next] ?? 'overview')}><TabList aria-label="Company research areas" activation="automatic" scrollIntoView>{tabs.map((item) => <Tab key={item}>{labels[item]}</Tab>)}</TabList><TabPanels>{tabs.map((item) => <TabPanel key={item}>{item === tab ? <div className="company-workspace__ledger"><KpiLedger metrics={metrics} /></div> : null}{item === tab ? panels[item] : null}</TabPanel>)}</TabPanels></Tabs></div></article></ScreenState>;
}
