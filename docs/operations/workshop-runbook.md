...
- Route the conflicting low-confidence candidate: `npm run insights:submit -- tests/fixtures/insights/candidate-low-conflicting.json --fixture-state tests/fixtures/insights/lifecycle-state.json --fixture-output-state /tmp/low-state.json`. Expect `queued`/`Needs Review`, while the prior publication remains.
